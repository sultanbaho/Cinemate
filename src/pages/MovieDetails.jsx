...
const image = `https://image.tmdb.org/t/p/w500/${details.poster_path}`
useEffect(() =>{
async function fetchdetails() {
  const response= await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=5e3e1547895ff3f0986240bc33933d02`);
...
useEffect(() =>{
  document.title=`Search Results For ${details.title}`;
})
  return (
    <main>
      <section className="flex justify-around flex-wrap py-5">
      <div className="max-w-sm">
          <img className="rounded" src={image} alt={details.title} />
        </div>
